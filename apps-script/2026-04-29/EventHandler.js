...
function handleMessageEvent(event) {
  ...
  try {
    ...
    const dfResponse =
      detectIntent(userId, userMessage);
    ...
    if (intentName === 'Check_Points') {
      ...
      const pointMsg =
        memberData
          ? `คุณ ${profile.displayName} มีคะแนนสะสม ${memberData.points.toLocaleString()} แต้มค่ะ 🐾`
          : "ไม่พบข้อมูลสมาชิกค่ะ";
    ...
    saveLog({
      ...
      botReply:
        fulfillmentText ||
        (hasPayload ? "Flex Message" : "")
    });

  } catch (error) {

    console.error(
      "❌ Error in handleMessageEvent:",
      error
    );
  }
}
 ...
 * ปรับปรุงเพื่อรองรับการตั้งค่า Welcome Message
 * และสลับ Rich Menu ตามสถานะสมาชิก
 */
...
function handleFollowEvent(event) {
  ...
  // 2. ✅ ตรวจสอบสถานะสมาชิก และสลับ Rich Menu ให้ถูกต้อง
  ...
  if (isMember) {
    ...
    // ลูกค้าใหม่ → ใช้เมนู Guest

    linkRichMenuToUser(
      userId,
      CONFIG.RICH_MENU_ID_VISITOR
    );
  }
